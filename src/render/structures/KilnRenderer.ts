...
export class KilnRenderer implements WorldRenderer {
  ...
  private bell(
    ...
    for (let i = 0; i <= cols; i++) {
      ...
      // shade by horizontal position (top-left lit) + a little patina mottle
      const lightAcross = 0.5 - fx * 0.5; // 1 at left, 0 at right
      for (let y = colTop; y < colBot; y += 3) {
        ...
        let col: number;
        if (lightAcross > 0.7) col = this.bronzeLight;
        else if (lightAcross > 0.4) col = this.bronzeBase;
        else col = mixColor(this.bronzeShade, this.patina, 0.35 + mottle * 0.2);
      ...
      if (lightAcross > 0.78) {
        p.block(x - cw / 2, colTop, cw, Math.min(8, colBot - colTop), this.bronzeLight, 0.4);
      }
    }
    ...
    // the flared lip line catching light
    ...
    for (let i = 0; i < 3; i++) {
      ...
      p.block(cx - w, y - crownH / 3, w * 2, crownH / 3 + 1, this.bronzeBase, 0.96);
      p.block(cx - w, y - crownH / 3, Math.max(1, w * 0.4), crownH / 3 + 1, this.bronzeLight, 0.5);
    }
  ...
  update(
    ...
    // ---- ambient still-pool glow under the bell ----------------------------
    this.fx.circle(LAYOUT.glowX, LAYOUT.glowY, 70).fill({
      color: mixColor(PALETTE.glow, this.accent.accentSoft, 0.3 + score * 0.3),
      alpha: 0.06 + score * 0.06 + 0.02 * Math.sin(t * 0.6),
    });
    ...
    const bellH = 110;
    const bellHalf = 56;
    const sway = Math.sin(t * 0.8) * 6 + Math.sin(t * 1.7) * 1.5; // struck sway
    ...
    for (let k = 1; k <= maxK; k++) {
      ...
      const pulse = Math.sin(t * 1.6 - k * 0.7) * (3 + a * 4);
      ...
      const baseAlpha = (0.12 + a * 0.45) * (0.7 + score * 0.5);

      // draw the ring as a stippled circle so it reads as "sound", not a hoop
      const segs = Math.max(28, Math.round(radius * 0.5));
      for (let s = 0; s < segs; s++) {
        const ang = (s / segs) * Math.PI * 2;
        // ripple the radius along the circumference for shimmer
        const rr =
          radius +
          ragged +
      ...
      // a soft glowing halo on the strongest rings
      if (a > 0.4) {
